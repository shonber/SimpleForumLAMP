...
class DatabaseInterface
{
    ...
    public function GetAllPosts($id)
    {
        try
        {
            ...
            {
				$post_data = htmlentities($obj["post_data"]);
                if ($obj["user_id"] == $id)
                {
					
                    $retval.="<li class='speech-bubble-right'><h2>{$obj["user_name"]}</h2><p>{$post_data}<a style='float:right;color: white;margin-right: 20px;' href='#' onclick='post_edit(this);'>edit</a></p><input value='{$obj["post_id"]}' hidden></li>";
                }
                else
                {
                    $retval.="<li class='speech-bubble-left'><h2>{$obj["user_name"]}</h2><p>{$post_data}</p><input name='post_id' value='{$obj["post_id"]}' hidden></li>";
                }
            }
